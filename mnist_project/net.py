...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        # 第一层卷积层：输入通道1（灰度图像），输出通道16，卷积核大小3x3
        ...
        # 第一个全连接层：输入特征数32*5*5（经过两次池化后的特征图大小），输出120
        # 计算过程：28x28输入 → 卷积后26x26 → 池化后13x13 → 卷积后11x11 → 池化后5x5
        self.fc1 = nn.Linear(32 * 5 * 5, 120)
        # 第二个全连接层：输入120，输出84
        self.fc2 = nn.Linear(120, 84)
        # 第三个全连接层：输入84，输出10（MNIST的类别数）
