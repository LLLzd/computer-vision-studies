...
def visualize_samples(images, labels, classes, save_path):
    ...
    # make_grid 会将单通道图像转换为 3 通道
    # 如果是 3 通道，取第一个通道（因为是灰度图，三个通道值相同）
    if npimg.shape[0] == 3:
        npimg = npimg[0]  # 从 [3, H, W] 变为 [H, W]
    else:
        npimg = npimg.squeeze()  # 从 [1, H, W] 变为 [H, W]
    ...
    # 设置标题，显示 "MNIST Samples" 和标签文本
    plt.title(f"MNIST Samples\n{label_text}", fontsize=12)
