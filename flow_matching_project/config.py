...
NUM_TIMESTEPS = 200  # 增加积分步数以提高生成质量

# 积分方法：ODE 求解器的类型
# 'euler': 欧拉方法，最简单但精度较低
# 'rk4': 四阶龙格-库塔方法，精度更高但计算量更大
INTEGRATION_METHOD = 'rk4'  # 使用 RK4 以提高生成质量
...
HIDDEN_DIM = 512  # 增加隐藏层维度以提高模型容量

# 隐藏层数量：网络的深度
# 更多层 = 更深的网络 = 可以学习更复杂的模式
NUM_HIDDEN_LAYERS = 6  # 增加隐藏层数量

# 时间嵌入维度：将时间 t 编码为高维向量
# 时间嵌入帮助网络理解当前处于流的哪个阶段
TIME_EMBED_DIM = 128  # 增加时间嵌入维度
...
BATCH_SIZE = 256  # 增加批次大小以提高训练稳定性

# 学习率：优化器更新权重的步长
# 太大：训练不稳定，可能发散
# 太小：训练太慢，可能陷入局部最优
LEARNING_RATE = 5e-4  # 降低学习率以配合更多 epoch

# 训练轮数：完整遍历数据集的次数
# 更多轮数 = 更充分的训练，但可能过拟合
NUM_EPOCHS = 300  # 增加训练轮数以获得更好的生成质量
...
# 权重衰减：L2 正则化系数，防止过拟合
# 通过惩罚大的权重值来提高泛化能力
WEIGHT_DECAY = 1e-4  # 增加权重衰减以防止过拟合
...
# 保存间隔：每多少步保存一次模型检查点
SAVE_INTERVAL = 20  # 每 20 个 epoch 保存一次

# 采样间隔：每多少步生成一次样本用于可视化
SAMPLE_INTERVAL = 5  # 每 5 个 epoch 采样一次
